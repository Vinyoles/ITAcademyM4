	...
	public static void main(String[] args) { 
		...
		*/
		...
		while (voleuAlgoMes == true) {
			...
			comanda.add(sca.nextLine());
			
			System.out.println("Voleu algo més? (y/n)");
		...
		//busca els elements demanats a l'array amb el menú. En el cas de que existeixen, els cobra. En el cas de que no surt un missatge d'error
		preuTotalMenjar = 0;
		for (int i=0; i<comanda.size(); i++) {
			boolean elementComandaAlMenu = false;
			for (int j=0; j<menu.length; j++) {
				if (menu[j].equals(comanda.get(i))) {
					preuTotalMenjar += preu[j];
					elementComandaAlMenu = true;
					break;
				}
			}
			
			//al final de la comprovació de cada plat de la comanda, retorna un missatge d'error si aquest no està disponible
			try {
				if (elementComandaAlMenu == false) {
					comanda.get(1000); //condició impossible de complir
				}
			} catch (Exception e) {
				System.out.println("El plat " + comanda.get(i) + " no està ofertat al menú");
			}			
		}
